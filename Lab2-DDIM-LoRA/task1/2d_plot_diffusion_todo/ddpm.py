...
class DiffusionModule(nn.Module):
    ...
    def ddim_p_sample(self, xt, t, t_prev, eta=0.0):
        ...
        if eta > 0:
            noise = torch.randn_like(xt)
            x_t_prev = torch.sqrt(alpha_prod_t_prev) * x0_pred + direction_coeff * predicted_noise + sigma_t * noise
        else:
            x_t_prev = torch.sqrt(alpha_prod_t_prev) * x0_pred + direction_coeff * predicted_noise
